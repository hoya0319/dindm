...
function draw(){
    ...
    if(headline.includes('해일의 관측치를 공지합니다.')){
        headline = `${(mon_day(tsunamiData.targetDateTime)).slice(4,)} 현재의 ${headline}`
    }
    ...
    if(tsunamiData.body.varComments){
        var p = document.createElement('p')
        p.textContent = tsunamiData.body.varComments;
        p.style="font-size: 1rem; white-space: pre-wrap; margin-top: 1rem"
        quakeDiv.appendChild(p)
    }
        ...
        region.stations.forEach(stationData => {
            ...
            if (matchedStation) {
                try{
                    ...
                    rectangle.bindPopup(`
                    <h4 style="font-family: 'Pretendard Variable'; text-align: center;">${matchedStation.name}</h4>

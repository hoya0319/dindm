...
var now_typhoon_center = L.icon({
    iconUrl: '/bousaihub/disaster/typhoon/typhoon.svg',
    iconSize: [25, 25]
})
...
function draw(nowData) {
    ...
    yellow_circle.on('click', function() {
        var popupContent = `
            <div>
                <p style="font-family: 'Pretendard-Regular'; text-align: center; font-weight:bold; font-size:1rem; margin:0">${nowData.body.info.now.classification.category}</p>
                <p style="font-family: 'Pretendard-Regular'; text-align: center;font-size:0.8rem; margin: -5px">${mon_day(nowData.body.info.now.dateTime).slice(3, -3)} 현재</p>
    ...
    for (var j = 0; j < (nowData.body.info.forecast).length; j++) {
        ...
        if (forecastData.isSuitei == false) {
            ...
            posibility_Circle.on('click', function() {
                var popupContent = `
                    <div>
                        <p style="font-family: 'Pretendard-Regular'; text-align: center; font-weight:bold; font-size:1rem; margin:0">${forecastData.classification.category}</p>
                        <p style="font-family: 'Pretendard-Regular'; text-align: center;font-size:0.8rem; margin: -5px">${mon_day(forecastData.dateTime).slice(3, -3)} 예보</p>
                        <table style="font-family: 'Pretendard-Regular'">
                        <tr>
                            <th style='text-align: center;'>중심기압</th>
        ...
        if (res.length == 0) {
            ...
            for (var i = 0; i < res.length; i++) {
                ...
                button.addEventListener('click', () => {
                    ...
                    var url = protocal + '//' + hostname + `/bousaihub/disaster/typhoon/?id=${nowData.eventID}`
            ...
            button.addEventListener('click', () => {
                var protocal = window.location.protocol;
                var hostname = window.location.host;
                var url = protocal + '//' + hostname + `/bousaihub/disaster/typhoon/`
                window.location.href = url;
            });
            document.getElementById('buttonContainer').appendChild(button);
        }
